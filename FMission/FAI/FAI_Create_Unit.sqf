/*
	...
	4. unit loadout type (Bandit, Sniper, Medic)
	5. Patrol Position
	
	Example:
	_aiunit = [_aiunit_spawn_pos,50,4,"Bandit",_loot_pos] execVM "\z\addons\dayz_server\addons\FMission\FAI\FAI_Create_Unit.sqf";
			
*/

private ["_aiunit","_xpos","_ypos","_unitpos","_aigroup","_wppos","_wpradius","_wpnum","_unit_type","_unit_loot_loadout","_unit_skill","_wp","_aispawnpos","_chance","_diceroll","_patrol_position"];
...
// Create AI	
    if (_unit_type == "Bandit") then 
    {
		...
		_null = [_aiunit] execVM "\z\addons\dayz_server\addons\FLoot\FLoot_Bandit.sqf";
	};
	...
	if (_unit_type == "Sniper") then 
    {
		...
		_null = [_aiunit] execVM "\z\addons\dayz_server\addons\FLoot\FLoot_Sniper.sqf";
	};
	...
	if (_unit_type == "Medic") then 
    {
		...
		_null = [_aiunit] execVM "\z\addons\dayz_server\addons\FLoot\FLoot_Medic.sqf";
	};	
...
diag_log format ["FAI Unit 1.0: -=%1=- %2 Spawned @ Location: %3 | Patrol Radius: %4m | Waypoints Assigned: %5",_unit_skill,_unit_type,_aispawnpos,_wpradius,_wpnum];
...
diag_log("FAI Unit 1.0: AI Units Deleted");